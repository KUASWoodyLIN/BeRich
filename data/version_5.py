...
def load_data(stock_sql, load_dir=''):
    ...
    if os.path.isfile(x_train_file_1) and os.path.isfile(x_train_file_2) and os.path.isfile(y_train_file) and \
        ...
        for stock_id in stock_ids:
            ...
            select = "SELECT 日期, 開, 收, MA10, MA20, MA60, UB20, wMA20, \
                      K9, OSC, D9, PB, BW, 成交量, 成交量'(流通比)', MV5, \
            ...
            # Create X Input 1
            x_data_1 = np.array(values)[:, 1:16].astype(np.float)
            ...
            # Create X Input 2
            x_data_2 = np.array(values)[:, 16:].astype(np.float)
